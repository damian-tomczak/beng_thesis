...
\label{sec:registry}
\hspace{\parindent} The purpose of Registry is to be so kind of database, it is achieved by storing all needed systems, entities' data.\\
...
Entity \hyperref[sec:registry]{\ref*{sec:registry} ECS Entity} described in the theoretical section should be represented only by ID in the form of the integer, but in our implementation an entity contains also a pointer to the registry it belongs.
...
The process of adding a PBR renderer component to an entity presents the next code snippet \hyperref[lst:renderer_component_pbr]{\ref*{lst:renderer_component_pbr} \texttt{PBRRendererComponent}}.
...
We didn't have time to provide true not breaking ABI ECS, the only protection has the form of inline namespaces with version - more about it in \hyperref[sec:abi]{\ref*{sec:abi} Building of TSEngine - Common ABI} section.\\
