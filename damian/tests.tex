...
\begin{lstlisting}[language=c++, caption=mat4rotationTest test(./engine/tests/tester.cpp)]
...
\end{lstlisting}
        \end{itemize}
    \item \texttt{GameTests}\\
    This category of the tests checks the basis game engine API functionalities.
    For this purpose, must be created a Game class simulating an example of game, but using \hyperref[sec:tester_adapter]{the tester adapter} to have a possibility to destroy rendering instance after some period of time, removing the need of human interaction while testing.
